...
//insertion po
export const insertNewPo = async po => {
  ...
  realm.write(() => {
    ...
    for (let i = 0; i < po.length; i++) {
      res[i] = realm.create("Po", po[i]);

      console.log(`Po created ** : ${res[i].POID} `);
    }
  });
};
...
//Retourner tous les documents(po) stockés dans la base realm locale
export const queryAllPos = async () => {
  const realm = await Realm.open(databaseOptions);
  console.log(' ****** opened to read ****************');

  const allServices = realm.objects("Po");
  /* console.log(
     `The lists of services are: ${allServices.map(srv => srv.TICKETID)}`,
   );*/
  return allServices;
